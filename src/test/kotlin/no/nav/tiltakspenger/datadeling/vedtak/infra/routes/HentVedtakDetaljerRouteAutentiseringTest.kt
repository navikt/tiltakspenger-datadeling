...
internal class HentVedtakDetaljerRouteAutentiseringTest {
    ...
    fun `post med gyldig token skal gi 200`() {
        with(TestApplicationContext()) {
            val tac = this
            val vedtakServiceMock = mockk<HentVedtakDetaljerService>().also { mock ->
                val virkningsperiode = 1 til 31.januar(2021)
                coEvery { mock.hentVedtakDetaljer(any(), any()) } returns listOf(
            ...
            testApplication {
                configureTestApplication(hentVedtakDetaljerService = vedtakServiceMock, texasClient = tac.texasClient)
                val response = client.post("/vedtak/detaljer") {
