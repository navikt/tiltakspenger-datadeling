...
class HentVedtakPerioderRouteTest {
    ...
    fun `hent vedtaksperioder - har vedtak fra arena og tpsak - riktig respons`() {
        with(TestApplicationContext()) {
            withMigratedDb { testDataHelper ->
                ...
                val vedtakService = HentVedtakPerioderService(vedtakRepo, arenaClient, fixedClock)
                    ...
                    application {
                        ...
                        routing {
                            authenticate(IdentityProvider.AZUREAD.value) {
                                hentVedtakPerioderRoute(hentVedtakPerioderService = vedtakService)
                            }
                        }
                    }
    ...
    fun `test at uten gyldig ident gir feilmelding`() {
        with(TestApplicationContext()) {
            ...
            val vedtakService = mockk<HentVedtakPerioderService>(relaxed = true)
            testApplication {
                application {
                    jacksonSerialization()
                    setupAuthentication(texasClient)
        ...
        with(TestApplicationContext()) {
            ...
            testApplication {
                application {
                    ...
                    routing {
                        authenticate(IdentityProvider.AZUREAD.value) {
                            hentVedtakPerioderRoute(hentVedtakPerioderService = mockk(relaxed = true))
                        }
                    }
                }
                defaultRequestMedKontraktsverifisering(
    ...
    fun `hent vedtaksperioder - feil fra arena - returnerer 500 server_feil`() {
        with(TestApplicationContext()) {
            ...
            val vedtakService = HentVedtakPerioderService(mockk(relaxed = true), arenaClient, fixedClock)
            testApplication {
                application {
                    jacksonSerialization()
                    setupAuthentication(texasClient)
    ...
    fun `hent vedtaksperioder - ukjent felt i request ignoreres - 200`() {
        with(TestApplicationContext()) {
            withMigratedDb { testDataHelper ->
                val arenaClient = mockk<ArenaClient>()
                coEvery { arenaClient.hentVedtak(any(), any()) } returns suksessRespons(emptyList())
                val vedtakService = HentVedtakPerioderService(testDataHelper.vedtakRepo, arenaClient, fixedClock)
                val token = systembrukerTokenMedLesVedtak()
                testApplication {
                    configureTestApplication(
                        hentVedtakPerioderService = vedtakService,
