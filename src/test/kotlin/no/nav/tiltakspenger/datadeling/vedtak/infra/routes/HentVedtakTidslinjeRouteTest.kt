...
class HentVedtakTidslinjeRouteTest {
    ...
    fun `hent tidslinje og vedtak - har to innvilgelser, stans og avslag, et arenavedtak - riktig respons`() {
        with(TestApplicationContext()) {
            withMigratedDb { testDataHelper ->
                ...
                val vedtakService = HentVedtakTidslinjeService(vedtakRepo, arenaClient, fixedClock)
    ...
    fun `test at uten gyldig ident gir feilmelding`() {
        with(TestApplicationContext()) {
            val tac = this
            val token = getGyldigToken()
            val vedtakService = mockk<HentVedtakTidslinjeService>(relaxed = true)
            testApplication {
                configureTestApplication(
                    hentVedtakTidslinjeService = vedtakService,
                    texasClient = tac.texasClient,
