...
class MottaNyttVedtakPostgresRepoTest {
    ...
    fun `kan lagre og hente vedtak`() {
        withMigratedDb { testDataHelper ->
            ...
            repo.hentForFnrOgPeriode(vedtak.fnr, Periode(vedtak.periode.fraOgMed, vedtak.periode.fraOgMed), "tp") shouldBe listOf(vedtak)
            // periode siste dag i vedtak
            repo.hentForFnrOgPeriode(vedtak.fnr, Periode(vedtak.periode.tilOgMed, vedtak.periode.tilOgMed), "tp") shouldBe listOf(vedtak)
            // periode etter vedtak
            repo.hentForFnrOgPeriode(vedtak.fnr, Periode(enDagEtterTilOgMed, enDagEtterTilOgMed), "tp") shouldBe emptyList()
        }
    }
}
