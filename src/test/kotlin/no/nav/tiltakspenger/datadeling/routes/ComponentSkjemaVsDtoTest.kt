...
import no.nav.tiltakspenger.datadeling.behandling.datadeling.routes.BehandlingRequest
...
import no.nav.tiltakspenger.datadeling.vedtak.datadeling.routes.VedtakDetaljerResponse
...
internal class ComponentSkjemaVsDtoTest {
    ...
    fun `hvert component-skjema matcher sin DTO`(): List<org.junit.jupiter.api.DynamicNode> {
        ...
        val tester = skjemaTilKlasse.map { (skjemaNavn, kClass) ->
            ...
            dynamicContainer(
                skjemaNavn,
                listOf(
                    dynamicTest("property-navn") {
                        val spec = samleSkjema(skjemaer.getValue(skjemaNavn), skjemaer)
                        assertEquals(
                            dtoFelter.keys.toSortedSet(),
                            spec.properties.keys.toSortedSet(),
                            "Property-navn mismatch for skjema $skjemaNavn vs ${kClass.simpleName}",
                        )
                    },
                    dynamicTest("required-sett") {
                        val spec = samleSkjema(skjemaer.getValue(skjemaNavn), skjemaer)
                        ...
                        // feltet kan utelates.
                        val alleFelter = dtoFelter.keys.toSortedSet()
                        assertEquals(
                            alleFelter,
                            spec.required.toSortedSet(),
                            "Required mismatch for $skjemaNavn: alle properties skal være required " +
                                "(Jackson utelater ikke null-felt). Nullability uttrykkes på typen.",
                        )
                    },
    ...
    private fun samleSkjema(
        ...
        val props = mutableMapOf<String, Map<*, *>>()
    ...
    private fun typeFeil(felt: String, specType: Map<*, *>, dtoType: KType): List<String> {
        ...
        when (type) {
            ...
            "object" -> {
                ...
                // Inline nested objects uten additionalProperties er ikke dypvaliderte –
                // disse fanges ofte av property-navn/required-sjekkene på den nestede klassen
                // dersom den senere promoteres til et eget skjema.
            }
        }
