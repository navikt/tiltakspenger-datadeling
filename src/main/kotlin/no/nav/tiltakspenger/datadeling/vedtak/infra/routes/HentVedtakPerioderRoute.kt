 ...
 *
 * Response-DTO: [VedtakDTO]
 */
fun Route.hentVedtakPerioderRoute(
    hentVedtakPerioderService: HentVedtakPerioderService,
) {
    ...
    post("/vedtak/perioder") {
        medSystembruker(Systembrukerrolle.LES_VEDTAK) { systembruker ->
            call.receive<VedtakReqDTO>().toFnrOgPeriode().fold(
                { error ->
                    logger.debug { "Systembruker ${systembruker.klientnavn} fikk 400 Bad Request mot POST /vedtak/perioder. Underliggende feil: $error" }
                    call.respond(HttpStatusCode.BadRequest, error)
                },
                { request ->
                    hentVedtakPerioderService.hentVedtakPerioder(
                        fnr = request.fnr,
