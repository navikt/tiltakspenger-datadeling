...
fun Route.arenaMeldekortRoutes(arenaMeldekortService: ArenaMeldekortService) {
    ...
    post("/arena/meldekort") {
        ...
        logger.debug { "Mottatt POST kall på /arena/meldekort - hent meldekort fra arena for fnr og periode - systembruker $systembruker" }

        if (!systembruker.roller.kanLeseMeldekort()) {
            logger.warn { "Systembruker ${systembruker.klientnavn} fikk 403 Forbidden mot /arena/meldekort. Underliggende feil: Mangler rollen ${Systembrukerrolle.LES_MELDEKORT}" }
