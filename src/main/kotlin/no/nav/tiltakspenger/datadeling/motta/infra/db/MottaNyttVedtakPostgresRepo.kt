...
internal class MottaNyttVedtakPostgresRepo(
            ...
            session.run(
                queryOf(
                    ...
                    mapOf(
                        ...
                        "rettighet" to vedtak.rettighet,
                        "kilde" to vedtak.kilde,
                        "mottatt_tidspunkt" to vedtak.kilde,
                    ),
                ).asUpdate,
            )
        }
    ...
    private fun eksisterer(vedtakId: String, kilde: String): Boolean {
        return sessionFactory.withSession { session ->
            session.run(
                queryOf(
                    "select count(*) from vedtak where vedtak_id = :vedtak_id and kilde = :kilde",
                    mapOf(
    ...
    fun hentForVedtakIdOgKilde(
        ...
        return sessionFactory.withSession { session ->
            session.run(
                queryOf(
                    "select * from vedtak where vedtak_id = :vedtak_id and kilde = :kilde",
                    mapOf(
                        "vedtak_id" to vedtakId,
                        "kilde" to kilde,
                    ),
